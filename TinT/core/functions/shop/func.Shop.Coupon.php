...
function tt_add_coupon($code, $type = "once", $discount = 0.9, $begin_date, $expire_date)
{
	...
	$expire_date = $expire_date ?: current_time("mysql");
	$insert = $wpdb->insert($coupons_table, array("coupon_code" => $code, "coupon_type" => $type, "discount_value" => $discount, "begin_date" => $begin_date, "expire_date" => $expire_date), array("%s", "%s", "%f", "%s", "%s"));
...
function tt_delete_coupon($id)
{
	...
	$prefix = $wpdb->prefix;
	$coupons_table = $prefix . "tt_coupons";
	$delete = $wpdb->delete($coupons_table, array("id" => $id), array("%d"));
	return !!$delete;
}
...
function tt_update_coupon($id, $data, $format)
{
	...
	$prefix = $wpdb->prefix;
	$coupons_table = $prefix . "tt_coupons";
	$update = $wpdb->update($coupons_table, $data, array("id" => $id), $format, array("%d"));
	return !$update === false;
}
...
function tt_check_coupon($code)
{
	...
	if (strtotime($coupon->expire_date) < $timestamp) {
