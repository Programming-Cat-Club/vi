...
HY_editor.prototype.init_ = function(selector){
	...
	for(var o in this.toolbar){
		...
		var $btn=$('<div class="hy-editor-'+(this.toolbar[o].name=='|' ? 'separator' : 'btn')+' '+ this.toolbar[o].btnClass+'" data-command="'+this.toolbar[o].command+'" data-name="'+this.toolbar[o].name+'" data-type="'+this.toolbar[o].type+'" data-nodename="'+this.toolbar[o].nodename+'" data-v="'+this.toolbar[o].v+'" unselectable="on" onmousedown="return false">'+
		...
		switch(type){
			...
			case 'select':
				...
				if($btn.hasClass('active-select')){
					...
					//移动下拉菜单位置
					//console.log(_this.nowToolFixed);
					if(_this.nowToolFixed){
						$select.css({
							left:$btn.position().left,
							top:  $btn.height() + $btn.position().top
						});
					}else{
						$select.css({
							left:$btn.position().left,
							top: ((0 - _this.$toolbar.height()) + $btn.height()) + $btn.position().top
						});
					}
